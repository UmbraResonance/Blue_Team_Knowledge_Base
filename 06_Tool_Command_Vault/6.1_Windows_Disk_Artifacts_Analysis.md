...
> **Tools:** EZ Tools (Zimmerman), Hindsight, bstrings.
...
**Search for a specific keyword (e.g., C2 Domain):**
```powershell
bstrings.exe -f "<PATH_TO_PAGEFILE.SYS>" --qs "<MALICIOUS_DOMAIN>"
# Example: bstrings.exe -f "C:\pagefile.sys" --qs "evil-site.com"
```

**Search using a standard Regex list (Credit Card, URL, Email):**
```powershell
bstrings.exe -f "<PATH_TO_PAGEFILE.SYS>" --lr "C:\Tools\bstrings\Regexes"
# Note: Can identify artifacts that no longer exist on disk (e.g., in-memory-only malware traces).
```