...
* **Key Fields to Analyze:**
    ...
    - **Ticket Encryption Type:** - `0x17` (RC4-HMAC): **CRITICAL RED FLAG.** Indicates a Kerberoasting attempt or a forged Golden Ticket signed with an NTLM hash.
        - `0x11`/`0x12` (AES): Modern standard; harder to crack but can still be used in advanced Golden Ticket attacks if the `krbtgt` AES key was stolen.
    - **Ticket Options:** Look for `0x40810000`. This is the default flag for tickets generated by Mimikatz/Rubeus.
    - **TargetUserName:** The account the ticket is issued for. In Golden Ticket attacks, this may be a non-existent user or a highly privileged one.
    - **IpAddress:** The source of the request.
* **Detection Logic (Integrated Kerberos Abuse Strategy):**
    ...
    - **Strategy B: The "Ghost" TGS Correlation (Golden Ticket Focus)**
        ...
        - **Indicator:** Detect instances of Event 4769 where no corresponding Event 4768 exists for that `TargetUserName` and `IpAddress` within the standard TGT lifetime (e.g., last 10 hours). This proves the TGT was forged offline.
    - **Strategy C: Ticket Anomaly & Encryption Downgrade**
        ...
        - Monitor for unusual `End Time` values. Forged tickets often have an extreme lifetime (e.g., 10 years by default in Mimikatz).
* **Reference:** 
