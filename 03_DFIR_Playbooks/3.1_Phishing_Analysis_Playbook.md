# Phishing Analysis Playbook (Universal)
...
* **Vault Link**: Use Linux text-processing utilities (grep/awk) to parse headers in bulk, as detailed in [`6.4_Linux_Live_Analysis`](../06_Tool_Command_Vault/6.5_Linux_Live_Analysis.md).
...
* **Hunting Link**: Reference [`1.4_Network_Protocol_Filters`](../01_Hunting_Cheatsheets/1.4_Network_Protocol_Filters.csv) for SMTP verification status.
...
### 1. Detection Pivot
* **Vault Link**: Apply [`6.7_Splunk_SPL_Syntaxes`](../06_Tool_Command_Vault/6.8_Splunk_SPL_Syntaxes.md) to correlate endpoint traffic with discovered malicious IPs/Domains.
