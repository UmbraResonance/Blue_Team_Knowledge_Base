# 8.2.14_Active_Directory_ACL_Principles.md
...
* **ReadProperty / WriteProperty**: Permission to interact with specific fields like `mobile`, `memberOf`, or `servicePrincipalName` (SPN).
* **msDS-AllowedToActOnBehalfOfOtherIdentity**: A critical attribute for **Resource-Based Constrained Delegation (RBCD)**. Writing to this property allows an attacker to specify which accounts can impersonate users to this computer.
...
| **WriteProperty (on RBCD)** | Delegation Manipulation. | Modifying `msDS-AllowedToActOnBehalfOfOtherIdentity` to execute an **RBCD** attack. |
...
* **Event ID 5136 (Directory Service Object Modified)**: The "Gold Standard" for AD changes.
    * **Focus 1**: Look for changes to the `nTSecurityDescriptor` attribute (ACL tampering).
    * **Focus 2**: Monitor changes to high-risk attributes like `msDS-AllowedToActOnBehalfOfOtherIdentity` or `servicePrincipalName`.
...
### **4.3 Detection Logic: "The Delegation Backdoor"**
1. **Trigger**: Event ID 5136 where `AttributeName` is `msDS-AllowedToActOnBehalfOfOtherIdentity`.
2. **Red Flag**: Any modification to this attribute on a sensitive Tier-0 or Tier-1 computer object by an account that is not part of a recognized administrative group.
...
- [[8.2.12_Kerberos_Delegation_Mechanics]]: Detailed mechanics of RBCD and constrained delegation.